...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  $('heroYoutubeBtn').onclick=playCurrentFromButton; $('heroSpotifyBtn').onclick=openSpotify;
...
function selectSong(i,autoplay=false){currentIndex=i;const s=SONG_LIST[i];$('cassetteTitle').textContent=s.title.toUpperCase();$('cassetteArtist').textContent=`${s.artist.toUpperCase()} · ${s.year}`;$('displayArtist').textContent=s.artist;$('displayTitle').textContent=s.title; if(playerReady&&player&&s.youtubeId){player.loadVideoById(s.youtubeId);if(!autoplay)player.pauseVideo()} else if(autoplay&&!s.youtubeId){showSourceStatus('No verified YouTube source for this song yet')} }
function onYouTubeIframeAPIReady(){player=new YT.Player('player',{videoId:SONG_LIST.find(s=>s.youtubeId)?.youtubeId||'',playerVars:{autoplay:0,controls:0,rel:0,playsinline:1},events:{onReady:()=>{playerReady=true},onError:()=>showSourceStatus('This YouTube source cannot be embedded. Please choose another song.'),onStateChange:e=>{$('playBtn').textContent=e.data===YT.PlayerState.PLAYING?'Ⅱ':'▶';if(e.data===YT.PlayerState.ENDED)next()}}})}
function togglePlay(){const s=SONG_LIST[currentIndex];if(!s)return;if(!s.youtubeId){showSourceStatus('No verified YouTube source for this song yet');return}if(!playerReady)return;const st=player.getPlayerState();st===YT.PlayerState.PLAYING?player.pauseVideo():player.playVideo()}
...
function playCurrentFromButton(){const s=SONG_LIST[currentIndex];if(!s)return;if(!s.youtubeId){showSourceStatus('No verified YouTube source for this song yet');return}selectSong(currentIndex,true)}
function openYoutube(){playCurrentFromButton()}
function showSourceStatus(message){$('displayTitle').textContent=message}
