...
function loadCurrentSource(autoplay){const s=SONG_LIST[currentIndex],id=s?.youtubeIds?.[sourceIndex];if(!id){showStatus('YouTube source unavailable');return}if(failedSources.has(id)){showStatus('Verified YouTube source unavailable');return}try{player.loadVideoById({videoId:id,startSeconds:0});if(!autoplay)player.pauseVideo();else player.playVideo()}catch(e){sourceIndex++;loadCurrentSource(autoplay)}}
function onYouTubeIframeAPIReady(){player=new YT.Player('player',{height:'1',width:'1',videoId:'',playerVars:{autoplay:0,controls:0,rel:0,playsinline:1,modestbranding:1},events:{onReady:()=>{playerReady=true},onError:()=>{const id=SONG_LIST[currentIndex]?.youtubeIds?.[sourceIndex];if(id)failedSources.add(id);showStatus('This verified song source cannot be embedded by YouTube.')},onStateChange:e=>{if(e.data===YT.PlayerState.PLAYING){$('playBtn').textContent='Ⅱ';$('displayTitle').textContent=SONG_LIST[currentIndex].title}else if(e.data===YT.PlayerState.PAUSED){$('playBtn').textContent='▶'}else if(e.data===YT.PlayerState.ENDED){$('playBtn').textContent='▶';next()}},onPlaybackQualityChange:()=>{}}})}
